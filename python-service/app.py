...
def analyze_bti():
    ...
    try:
        ...
        3. ТОЧКИ СЪЕМКИ И ВИЗУАЛЬНЫЕ ОРИЕНТИРЫ (camera_points):
        - ПРАВИЛО ПРИВЯЗКИ: Каждая точка должна быть привязана к ГРАФИЧЕСКОМУ СИМВОЛУ, который виден на плане.
        - СПИСОК ДОПУСТИМЫХ ОБЪЕКТОВ (в порядке приоритета):
         1. Дверной проем (пунктирная дуга или разрыв в стене).
         2. Окно (тонкая линия внутри толстой стены) — ТОЛЬКО если оно есть в этом помещении.
        3. Сантехника (символы ванны, унитаза, раковины) — для мокрых зон (санузлы).
         4. Кухонная плита (квадрат с кружками) — для кухни.
         5. Угол помещения — если других объектов нет.

        - ФОРМАТ ОПИСАНИЯ:
        * location: "Точка у [Название объекта на плане]"
        * view: "В сторону [Противоположный объект или угол]"

         - ПРИМЕР ДЛЯ САНУЗЛА (без окон):
        {"point_id": 1, "location": "У дверного проема", "view": "На ванну и раковину"},
        {"point_id": 2, "location": "У унитаза", "view": "На входную дверь"}
        """
