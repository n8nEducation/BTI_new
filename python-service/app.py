...
def step_1_ocr_analysis(image_base64, target_area=None):
    ...
    prompt = (
        "Ты — робот-сканер БТИ. Твоя задача — ПЕРЕПИСАТЬ данные с чертежа.\n\n"
        "СТРОГИЕ ПРАВИЛА ПОИСКА ПЛОЩАДИ:\n"
        "1. ПЛОЩАДЬ — это обычно цифра в центре комнаты, часто стоит под номером помещения или имеет пометку 'м2'.\n"
        "2. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО брать цифры, расположенные вдоль линий стен (это длины стен, а не площадь).\n"
        "3. Если в комнате НЕТ явной цифры площади — пиши null в поле 'area'. Не пытайся подставить длину стены вместо площади.\n"
        f"4. Если дана общая площадь квартиры ({str(target_area) if target_area else 'не дана'}), и в комнате нет цифры — можешь вычислить её остаточным методом, но пометь это.\n\n"
        "ПРАВИЛА ИМЕНОВАНИЯ:\n"
        "- Если текста нет — пиши 'Помещение'. ЗАПРЕЩЕНО выдумывать названия.\n\n"
        "Верни JSON: {'is_plan': true, 'rooms': [{'id': '...', 'name': '...', 'area': float_or_null, 'shape': '...'}]}"
    )
