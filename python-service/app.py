...
- Используй ТОЛЬКО текст, который буквально написан на плане рядом с помещением (например, "Кухня", "Жилая", "Санузел").
- СТРОГО ЗАПРЕЩЕНО: додумывать, угадывать или предполагать название по форме, размеру или расположению помещения.
- Если рядом с помещением на плане НЕТ подписанного названия — пиши строго: "Помещение [id]" (где [id] — номер этого помещения).
